...
import ModalAgregarProveedor from "../../components/consts/ModalDinamico";
...
const Proveedores = () => {
  ...
  const handleAddProveedor = async (formData) => {
    try {
      ...
      const proveedorExistenteCorreo = proveedores.find(
        (proveedor) => proveedor.correo_proveedor === correo_proveedor
      );
      const proveedorExistenteTelefono = proveedores.find(
        (proveedor) => proveedor.telefono_proveedor === telefono_proveedor
      );
      const proveedorExistenteDireccion = proveedores.find(
        (proveedor) => proveedor.direccion_proveedor === direccion_proveedor
      );
      const proveedorExistenteEmpresa = proveedores.find(
        (proveedor) => proveedor.empresa_proveedor === empresa_proveedor
      );
      ...
      if (confirmation.isConfirmed) {
        ...
        await axios.post(
          "http://localhost:5000/api/proveedores/guardarProveedor",
          formData
        );
  ...
  const handleEditProveedor = async (formData) => {
    try {
      ...
      const proveedorExistenteEmpresa = proveedores.find(
        (proveedor) =>
          proveedor.empresa_proveedor === empresa_proveedor &&
          proveedor.IdProveedor !== formData.IdProveedor
      );
      ...
      if (confirmation.isConfirmed) {
        formData.estado_proveedor = 1;
        await axios.put(
          "http://localhost:5000/api/proveedores/editar",
          formData
        );
  ...
  const handleToggleSwitch = async (id) => {
    try {
      ...
      const updatedProveedor = {
        ...proveedor,
        estado_proveedor: proveedor.estado_proveedor === 1 ? 0 : 1,
      };
      await axios.put(
        `http://localhost:5000/api/proveedores/editar`,
        updatedProveedor
  ...
  return (
    <div>
      <center>
        <h3 className="text-4xl mb-4" style={{ marginTop: "-20px" }}>
          Gestion Proveedores
        </h3>
      </center>
