...
import ModalAgregarCategoria from "../../components/consts/ModalDinamico";
...
const Categorias = () => {
    ...
    try {
      ...
      if (
        !CamposObligatorios(
          formData,
          camposObligatorios,
          "Por favor, complete todos los campos de la categoría."
        )
      ) {
        return;
      }
      ...
      if (confirmation.isConfirmed) {
        ...
        await axios.post(
          "http://localhost:5000/api/categorias/guardarCategoria",
          formData
        );
  ...
  const handleEditCategoria = async (formData) => {
    try {
      ...
      const categoriaExistente = categorias.find(
        (categoria) =>
          categoria.nombre_categoria === formData.nombre_categoria &&
      ...
      if (confirmation.isConfirmed) {
        await axios.put(
          "http://localhost:5000/api/categorias/editar",
          formData
        );
  ...
  const handleToggleSwitch = async (id) => {
    try {
      ...
      const updatedCategoria = {
        ...categoria,
        estado_categoria: categoria.estado_categoria === 1 ? 0 : 1,
      };
      await axios.put(
        `http://localhost:5000/api/categorias/editar`,
        updatedCategoria
  ...
  return (
    <div>
      <center>
        <h3 className="text-4xl mb-4" style={{ marginTop: "-130px" }}>
          Gestion Categorias
        </h3>
      </center>
      ...
      <ModalAgregarCategoria
        ...
        fields={[{ name: "nombre_categoria", label: "Nombre", type: "text" }]}
