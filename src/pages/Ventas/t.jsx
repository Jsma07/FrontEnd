...
const Registrar = () => {
    ...
    try {
      ...
      const updatedInsumos = insumosSeleccionados.map((insumo) => {
        ...
        if (cantidadVendida > 0) {
          const usosAUtilizar = Math.min(
            cantidadVendida,
            nuevosUsosDisponibles
          );

          nuevosUsosDisponibles -= usosAUtilizar;
          nuevaCantidadDisponible -= Math.ceil(cantidadVendida / usoUnitario); // Redondear hacia arriba

          console.log("Usos a utilizar:", usosAUtilizar);
  ...
  useEffect(() => {
    // Calcular el total general y actualizar el estado del IVA
    let total = 0;
    insumosSeleccionados.forEach((insumo) => {
      const subtotal =
        insumo.PrecioUnitario * (cantidadInsumos[insumo.IdInsumos] || 0);
